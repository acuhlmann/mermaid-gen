...
**Theme-first note:** Example snippets may include hardcoded `classDef` colors for illustration. For production output, prefer the configured theme and remove color overrides unless explicitly required.

## Input Artifacts to Inspect First

- `src/main/java/**/**Application.java` (bootstrap and auto-configuration root)
- `src/main/java/**/controller/**/*.java` (API surface and request flow)
- `src/main/java/**/service/**/*.java` and `**/repository/**/*.java` (domain/data boundaries)
- `src/main/resources/application*.yml` (profiles, infra endpoints, runtime config)
- `pom.xml` / `build.gradle` + `Dockerfile` / `k8s/*.yaml` (dependencies and deployment shape)
