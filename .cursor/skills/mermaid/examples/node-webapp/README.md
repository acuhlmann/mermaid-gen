...
- `src/server.js` / `src/app.js` (bootstrap, middleware chain, router mounting)
- `src/routes/**/*.js` (HTTP surface and route grouping)
- `src/controllers/**/*.js` and `src/services/**/*.js` (request-to-domain flow)
- `src/models/**/*.js` or ORM schema files (data model relationships)
- `Dockerfile`, `docker-compose.yml`, `ecosystem.config.js` (deployment/runtime layout)
