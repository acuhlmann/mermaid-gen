...
- `src/main.tsx` / `src/App.tsx` (app root, providers, routing entry)
- `src/components/**/*.{tsx,jsx}` (component hierarchy and composition)
- `src/pages/**/*.{tsx,jsx}` and `src/routes/**/*` (navigation and page boundaries)
- `src/store/**/*` / `src/context/**/*` (state management and side effects)
- `vite.config.*`, `webpack.config.*`, `Dockerfile` (build/runtime topology)
