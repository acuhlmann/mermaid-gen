...
- `src/main/java/**/*Servlet.java` and controllers (entry points and request flow)
- `src/main/java/**/service/**/*.java` and `**/dao/**/*.java` (use-case and data access flow)
- `src/main/webapp/WEB-INF/web.xml` (servlet/filter mappings)
- `src/main/webapp/WEB-INF/*.jsp` (view-level interaction points)
- `conf/server.xml` or container config + `Dockerfile` (runtime/deployment shape)
