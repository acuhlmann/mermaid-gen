...
- **Validating before finalizing** using the Node.js render script
...
flowchart TD
    ...
    Save --> Validate[Step 4: Validate with render.js]
...
- Use theme-first styling; avoid hardcoded `classDef fill/stroke/color` unless explicitly requested
...
./diagrams/<base_filename>.svg    # Rendered image
```

### Step 4: Validate with render.js

Run the render script to validate and render the diagram:

```bash
node "$PLUGIN_DIR/scripts/render.js" diagram.mmd --output diagram.svg
```
...
1. **Parse the error message** from render.js output
...
./diagrams/api_design_doc_01_sequence_user_authenticatio.svg
```
...
The `resilient_diagram.js` script automates the entire workflow:

```bash
# Basic usage
node "$PLUGIN_DIR/scripts/resilient_diagram.js" \
...
```json
{
  ...
  "image_path": "./diagrams/api_01_sequence_auth_flow.svg",
...
1. **Read the error message** from render.js output
...
If the `resilient_diagram.js` script is unavailable or fails, follow these manual steps:
...
```bash
node "$PLUGIN_DIR/scripts/render.js" "./diagrams/${FILENAME}" --output "./diagrams/${FILENAME%.mmd}.svg"
```
...
1. **Read error message** from render.js output
...
6. **Add to markdown:**
   ...
   ![User Login Sequence](./diagrams/auth_doc_01_sequence_user_login.svg)
   ```
...
4. VALIDATE → Run render.js, check exit code
5. RECOVER  → If failed: troubleshooting.md → search tools
6. ADD      → Only after validation: ![](./diagrams/name.svg)
```
...
./diagrams/<markdown>_<num>_<type>_<title>.svg
```
...
```bash
node "$PLUGIN_DIR/scripts/render.js" input.mmd --output output.svg
```
