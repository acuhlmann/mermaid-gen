...
2. **Validate syntax** using `node "$PLUGIN_DIR/scripts/extract_mermaid.js" <file> --validate`
...
node "$PLUGIN_DIR/scripts/extract_mermaid.js" docs/architecture.md --validate

# Convert to image to verify rendering
node "$PLUGIN_DIR/scripts/render.js" diagram.mmd --output output.svg
```

### 5. **Apply Theme-First Styling**

Before finalizing, keep the diagram compatible with the configured Mermaid theme.
Avoid hardcoded `classDef fill/stroke/color` unless custom node colors are explicitly required.
Use `skills/mermaid/references/guides/styling-guide.md` for minimal, targeted overrides.
...
### 7. **Use Semantic Unicode Symbols**
...
flowchart TD
    ...
    Generate --> Validate[✅ Validate Syntax<br/>scripts/extract_mermaid.js --validate]
...
- [ ] **Step 7:** Apply theme-first styling
