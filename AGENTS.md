...
- **Lint**: Only `apps/web` has an ESLint config (`npm run lint -w apps/web`). There are pre-existing warnings (0 errors, 36 React hooks-rule warnings as of May 2026).
- **Lockfile integrity**: The `package-lock.json` may become stale when `@copilotkit/runtime` updates its transitive deps (e.g. `@ai-sdk/*`, `reflect-metadata`). If the server fails at startup with `ERR_MODULE_NOT_FOUND`, delete `package-lock.json` and `node_modules` and run `npm install` to regenerate a complete lockfile.
