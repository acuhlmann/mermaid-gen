...
for entry in "${selected[@]}"; do
  ...
  loop=false
  [[ "$kind" == "bed" ]] && loop=true
  body=$(REQ_PROMPT="$prompt" REQ_SECS="$secs" REQ_INFL="$influence" REQ_LOOP="$loop" python3 -c '
import json, os
print(json.dumps({
  ...
  if [[ "$kind" == "bed" ]]; then
    ...
    gain=$(python3 -c "print(f'{$BED_TARGET_LUFS - ($measured):.1f}')")
    ...
    read -r start dur < <(ffmpeg -hide_banner -loglevel error -y -i "$raw" -ac 1 -ar 44100 \
      -f wav "$WORK_DIR/$name.probe.wav" && python3 - "$WORK_DIR/$name.probe.wav" <<'PY'
...
PY
    ...
    fade_out=$(python3 -c "print(round($dur - 0.03, 3))")
    ...
    gain=$(python3 -c "print(round($CUE_TARGET_PEAK_DB - ($pk), 2))")
