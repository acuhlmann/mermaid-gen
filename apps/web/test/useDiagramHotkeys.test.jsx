...
describe('useDiagramHotkeys', () => {
  ...
  it('does NOT fire when enabled is false', () => {
    const onAction = vi.fn();
    render(<Harness enabled={false} descriptor={{ id: 'n' }} onAction={onAction} onToggleHelp={vi.fn()} />);
    fireEvent.keyDown(window, { key: 'r' });
  ...
  it('fires onToggleHelp on ?', () => {
    ...
    render(<Harness enabled descriptor={{ id: 'n' }} onAction={vi.fn()} onToggleHelp={onToggleHelp} />);
    fireEvent.keyDown(window, { key: '?' });
  ...
  it('onToggleHelp fires even when disabled / no descriptor', () => {
    const onToggleHelp = vi.fn();
    render(<Harness enabled={false} descriptor={null} onAction={vi.fn()} onToggleHelp={onToggleHelp} />);
    fireEvent.keyDown(window, { key: '?' });
    expect(onToggleHelp).toHaveBeenCalledTimes(1);
  });
  ...
  it('does nothing on unmapped keys', () => {
    const onAction = vi.fn();
    const onToggleHelp = vi.fn();
    render(<Harness enabled descriptor={{ id: 'n' }} onAction={onAction} onToggleHelp={onToggleHelp} />);
