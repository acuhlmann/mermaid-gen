...
describe('RenderAsMascot', () => {
  ...
  it('lists all content modes in the radial-style menu when expanded (test mode)', () => {
    ...
    expect(
      screen.getByRole('button', { name: /Render selected item as 3D metaphor/i })
    ).toBeTruthy();
    ...
    expect(
      screen.getByRole('button', { name: /Render selected item as Anything page/i })
    ).toBeTruthy();
  ...
  it('invokes onPickMode and collapses when a different mode is chosen', () => {
    ...
    render(<RenderAsMascot modes={MODES} currentMode="mermaid" onPickMode={onPickMode} />);
  ...
  it('disables the current mode row', () => {
    render(<RenderAsMascot modes={MODES} currentMode="chart" onPickMode={vi.fn()} />);
