/** Traditional Chinese overrides for UI chrome. */
export const CONTROLS_ZH_TW = {
  actions: {
    definition: '這是什麼？',
    definitionPersona: '快速參考',
    definitionTitle: '快速參考 · 這個元素是什麼意思？',
    stakeholders: '相關方',
    stakeholdersTitle: '相關方 · 點擊召集圓桌',
    renderMode: '渲染為…',
    renderModePersona: '模式切換',
    renderModeTitle: '模式切換 · 用另一種模式重新渲染所選內容',
    gilfoyle: '精修',
    dinesh: '精修',
    erlich: '創新',
    russ: 'Russ',
    russHarder: '這傢伙會交',
    russHardest: '三個逗號',
    maxRuss: '無線電靜默',
    coDesign: '協同設計',
    jared: '評審',
    fix: '修復',
    fixPersona: '現場工頭',
    fixTitle: '現場工頭 · 修復這坨爛圖',
    richard: '講解',
    clear: '清空',
    clearTitle: '清空 · 申請拆除許可證，清掉工位重來',
    demolish: '碎紙機',
    facilities: '找物業',
    facilitiesTitle: '呼叫物業 · 修好這坨爛圖',
    prepForCeo: '協同',
    prepForCeoTitle: '給 CEO 準備稿 · 送上樓前先熬乾水分',
    connect: '新增',
    connectTitle: '新增連結框 · Shift+點擊可連到現有框',
    link: '連接',
    linkTitle: '連接 · 點另一個框以連線',
    delete: '刪除',
    deleteTitle: '刪除 · 從圖表中去掉這個',
    rename: '重新命名',
    renameTitle: '重新命名 · 改標籤'
  },
  prompt: {
    yourTopic: '你的主題',
    doIt: '開始',
    mic: '麥克風',
    holdToSpeak: '按住說話',
    holdToDictate: '按住語音輸入',
    tapToDictate: '點擊語音輸入',
    tapToDictatePrompt: '點擊語音輸入提示',
    tapToStop: '點擊停止聽寫',
    voiceNeedsHttps: '語音輸入需要安全連線（HTTPS），localhost 除外',
    voiceUnsupported: '此瀏覽器不支援語音輸入',
    slopNextTitle: '接下來要改什麼？',
    slopNextPlaceholder: '告訴智慧代理要改什麼…',
    slopNextLabel: '新提示',
    slopNextRadialTitle: '編輯此選取項',
    slopNextRadialTitleNamed: '編輯「{name}」',
    slopNextRadialPlaceholder: '重新命名、改標籤、補充細節…',
    slopNextRadialHint: '只改這一部分——圖表其餘部分保持不變。',
    closePrompt: '關閉提示',
    deskLabel: '工單 — 這次要改什麼？',
    deskPlaceholder: '寫下你的下一條工單…',
    topicPlaceholder: '輸入任何主題 — 圖表、資料圖、表單…',
    starterHint: '或從同事那裡接個任務：',
    starterAria: '用於快速開始的範例主題',
    entryIntro: {
      greeting: '工位魔法時間，{name}',
      role: '架構師',
      body: '你熬過了新人引導。這是最後一次來自公司的擁抱。在下面的工單裡拋個主題——每份交付物都是這麼開始的，你也是這麼從「擺設」變成「同事」的。'
    },
    entryPointers: [
      {
        id: 'work-order',
        label: '1 · 工單',
        text: '在這裡輸入你的主題，點「開始」產生。每份交付物都從這裡出發。'
      },
      {
        id: 'desk',
        label: '2 · 操作選單',
        text: '開啟郵件、匯出和工位操作。任何時候都能用「站起來」去辦公室樓層走一圈。'
      },
      {
        id: 'notebook',
        label: '3 · 筆記本',
        text: '產生交付物之後，這裡會顯示思考記錄、執行歷史和審查結果。'
      },
      {
        id: 'team',
        label: '4 · 你的團隊',
        text: '畫布上一有內容，你的團隊就會開始提建議、發審查意見。'
      },
      {
        id: 'selection',
        label: '5 · 精細編輯',
        text: '點擊畫布上任意節點、連線或標籤即可開啟操作環——裡面每個動作只改這一部分，不動整張圖。'
      },
      {
        id: 'format',
        label: '6 · 交付物格式',
        text: '選單列負責選格式——架構圖、數據圖、資訊圖、3D 或任意頁面。先選一個，再去工單裡寫主題。'
      }
    ],
    entryTour: {
      next: '下一步',
      skip: '略過導覽',
      done: '拋出你的第一個主題',
      deskEyebrow: '在你的工位上'
    },
    renderAsHint:
      '產生前先在選單列的「交付物」裡選一種格式。收發室負責把它寄出去；筆記本圖示開啟思考記錄。',
    starters: [
      { label: '咖啡供應鏈', prompt: '拆解全球咖啡供應鏈' },
      { label: 'OAuth 2.0 流程', prompt: '講解 OAuth 2.0 授權碼流程的運作原理' },
      { label: 'CI/CD 流水線', prompt: '梳理從提交到上線的 CI/CD 流水線' },
      { label: '微服務', prompt: '畫一張典型的微服務架構圖' }
    ],
    exampleEyebrow: 'ArchiSlop 公司 · IT 事業部',
    exampleHeadline: '歡迎入職，{name}',
    exampleRole: '架構師',
    exampleAria: '歡迎來到你的 ArchiSlop 工位',
    renderAsLabel: '渲染為',
    renderAsAria: '選擇主題的渲染形態'
  },
  dayOne: {
    eyebrow: 'ArchiSlop 公司 · 員工識別證',
    rolePrefix: '新進員工',
    hrLine: '識別證照片：還在處理中。座位：就是這張。股權：氛圍感。合規訓練：已經逾期（破紀錄）。',
    pitchLine:
      '這塊畫布就是你的交付物。在下方拋出任何主題——或者在你覺得自己夠 Series-A 勇敢時，接下同事的派活。',
    dismissAria: '收起識別證',
    nameTag: {
      hello: '你好',
      subtitle: '我叫',
      placeholder: '新人',
      editTitle: '輸入你的名字——整個辦公室都會開始這麼叫你',
      inputAria: '你在辦公室的名字'
    }
  },
  introLocale: {
    aria: '介面語言',
    en: '英語',
    enAu: '澳式俚語',
    zhCn: '簡體中文',
    zhTw: '繁體中文'
  },
  menuBar: {
    aria: '工作站選單列',
    settings: '設定',
    settingsTitle: '設定 · 趕工、深耕、外包、人資、耳機',
    settingsAria: '設定 — 工位偏好',
    deliverable: '交付物',
    deliverableTitle: '交付物 · 這份東西到底是什麼，以及怎麼碎掉它',
    deliverableAria: '交付物格式',
    formatSection: '交付物格式',
    view: '檢視',
    viewTitle: '檢視 · 程式碼抽屜、筆記本、全螢幕',
    viewAria: '面板與螢幕',
    admin: '行政',
    adminTitle: '行政 · 外包、人事、耳機、語言、快速鍵',
    adminAria: '行政 — 外包、人事、耳機、語言、快速鍵',
    hotkeys: '鍵盤快速鍵'
  },
  languagePack: {
    label: '語言包',
    tag: 'IT 工單',
    title: '向 IT 提工單，更換工位介面方言',
    aria: '語言包 — 向 IT 申請更換介面語言'
  },
  settings: {
    label: '設定',
    show: '顯示設定',
    hide: '隱藏設定',
    title: '工位 · 承包商與程式碼抽屜',
    region: '工作階段設定',
    outboxLabel: '收發室',
    outboxShow: '開啟收發室',
    outboxHide: '關閉收發室',
    outboxTitle: '收發室 · 儲存、複製或分享這份交付物',
    outboxRegion: '收發室 — 匯出與分享',
    outboxCounter: '3號窗口 · 寄出',
    externalAgents: '外部智慧代理',
    waitingHandshake: '等待握手：',
    externalAgentFallback: '外部智慧代理',
    brain: '專注度',
    fast: '趕工',
    fastShort: '趕工',
    quality: '深耕',
    qualityShort: '深耕',
    concentrationTitle: '下一份交付物前你要想多認真',
    mode: '模式',
    thinking: '筆記本',
    aiCluster: '工位',
    export: '匯出',
    exportEmpty: '暫無待寄 — 先產生一份交付物。',
    exportWorking: '正在匯出…',
    exportFailed: '匯出失敗',
    exportMermaidSource: 'Mermaid 原始碼 (.mmd)',
    exportMermaidSvg: 'SVG 圖像 (.svg)',
    exportInfographicDsl: '資訊圖 DSL (.txt)',
    exportInfographicPng: 'PNG 圖像 (.png)',
    exportMetaphorJson: '場景 JSON (.json)',
    exportMetaphorPng: 'PNG 截圖 (.png)',
    exportMetaphorUsda: 'USD 場景 (.usda)',
    exportChartCsv: '試算表 CSV (.csv)',
    exportChartJson: '圖表 JSON (.json)',
    exportChartPng: 'PNG 圖像 (.png)',
    exportChartVegaLite: 'Vega-Lite 規格 (.vl.json)',
    exportAnythingHtml: '獨立 HTML (.html)',
    exportFormsJson: '表單 JSON (.json)',
    exportFormsPng: 'PNG 截圖 (.png)',
    exportMermaidPng: 'PNG 圖像 (.png)',
    exportSave: '儲存',
    exportCopy: '複製',
    exportShare: '分享',
    exportSharePrimary: '分享',
    exportSharePreparing: '準備中…',
    exportActionsFor: '{label} 的操作',
    exportSaved: '已儲存到裝置',
    exportShared: '已分享',
    exportCopiedText: '已複製到剪貼簿',
    exportCopiedImage: '已複製圖像到剪貼簿',
    exportOpenPreview: '開啟預覽',
    exportCopyAgain: '再次複製',
    exportShareAgain: '再次分享',
    exportDismiss: '關閉',
    exportDownloadHint: '請查看通知列或 檔案 → 下載。'
  },
  contentModes: {
    auto: '自動',
    autoShort: '自動',
    autoSubtitle: '依主題自動選擇最合適的模式',
    mermaid: '架構圖',
    mermaidShort: '架構圖',
    mermaidSubtitle: 'Mermaid 架構關係圖',
    infographic: '資訊圖',
    infographicShort: '資訊圖',
    infographicSubtitle: 'AntV 敘事版面',
    metaphor3d: '3D 隱喻',
    metaphor3dShort: '3D',
    metaphor3dSubtitle: 'Three.js 空間場景',
    chart: '數據圖',
    chartShort: '數據圖',
    chartSubtitle: 'Vega-Lite 資料檢視',
    anything: '任意頁面',
    anythingShort: '任意',
    anythingSubtitle: 'HTML/CSS/JS 沙箱',
    forms: '表單',
    formsShort: '表單',
    formsSubtitle: '無盡的互動表單',
    anotherMode: '其他模式',
    renderMenu: '目標渲染模式'
  },
  modeReveal: {
    eyebrow: '同一主題，另一種形態',
    body: '隨時可在底部列的「渲染為」按鈕切換模式——把同一個想法渲染成 3D 場景、圖表、資訊圖或自由頁面。',
    pickPrefix: '渲染為',
    dismiss: '知道了',
    aria: '用另一種模式渲染你的主題'
  },
  hotkeys: {
    title: '鍵盤快捷鍵',
    close: '關閉快捷鍵說明',
    hint: '選中圖表元素後可使用單鍵快捷鍵。輸入文字時快捷鍵無效。',
    gilfoyle: 'Gilfoyle — 把本來就存在的畫出來',
    dinesh: 'Dinesh — 補上會出事的地方，順便記下是他補的',
    erlich: 'Erlich — 更大膽的重設計',
    russ: 'Russ — 主題升級',
    barker: 'Barker — 濃縮要點',
    jared: 'Jared — 標出流程缺口',
    richard: 'Richard — 命名模式',
    toggleHelp: '切換此說明',
    officeView: '站起 / 坐下（辦公室樓層）',
    esc: '關閉選單 / 對話框',
    arrows: '在環形操作中移動焦點',
    activate: '啟用目前焦點操作'
  },
  clearDialog: {
    prompts: [
      {
        title: '🚧 拆除許可申請中',
        body: '確定要推平這件傑作嗎？這坨爛圖只會留在我們的記憶裡（大概還有三份 Confluence 頁面）。'
      },
      {
        title: '🏗️ 拆遷球待命',
        body: '你確定嗎？一旦拆掉，相關方至少要開一次復盤，再來一輪組織調整。'
      },
      {
        title: '⛏️ 準備碾成碎石？',
        body: '拆除圖表會重置一切——包括我們一連串勇敢的架構決策連擊。'
      },
      {
        title: '💣 控制性爆破？',
        body: '架構本應永恆，除非你按了按鈕。最後機會，保住這坨爛圖。'
      }
    ],
    save: '保住爛圖',
    saveAria: '保住爛圖',
    demolish: '拆掉它！',
    demolishAria: '拆掉它'
  },
  explainDumb: {
    rephrasePlain: '用更直白的語言重述 — 再點一次會更簡單',
    rephraseYounger: '為更年輕的讀者再簡化一點',
    rephraseGibberish: '最後一試：前語言胡話',
    decommissionTitle: '下線此講解（超出範圍）',
    decommissionAria: '我放棄了 — 下線此講解',
    rephraseAriaSuffix: ' — 為更簡單的讀者重述',
    surrenderCaption: '已移入架構待辦。不修。 🏳️',
    rephraseGroup: '重述選項',
    gibberishActive: '胡話簡化已啟用'
  },
  radial: {
    selectionActions: '針對所選圖表部分的精細編輯',
    chipHint: '僅改此部分',
    introEyebrow: '🎯 精細編輯',
    introBody:
      '你選中了一個圖表元素。此處的每個操作只改這一部分——其餘保持不變。用「編輯」輸入你的要求，或交給團隊處理。',
    introDismiss: '知道了',
    introAria: '精細編輯說明',
    whatDoesMean: '{target} 是什麼意思？',
    whatDoesThisMean: '這是什麼意思？',
    dragToReposition: '拖動以重新定位',
    decommissioning: '正在下線',
    wiseArchitect: '資深架構師',
    schedulingDeprecation: '正在為以下項目安排下線',
    wiseArchitectOn: '資深架構師講解',
    thisElement: '此元素',
    closeExplanation: '關閉講解',
    drillDeeper: '深入挖掘',
    drillDeeperTitle: '在思考面板中展開完整架構深潛',
    explanationMissing: '沒有返回講解 — 請稍後再試。',
    explanationFailed: '無法取得講解。',
    stakeholdersForElement: '此元素的相關方',
    stakeholdersHeading: '相關方',
    stakeholdersWithName: '相關方 · {name}',
    closeStakeholders: '關閉相關方',
    renderSelectedInMode: '用另一種模式渲染所選項目',
    renderAsHeading: '渲染為…',
    renderNameAs: '將 {name} 渲染為…',
    closeRenderPicker: '關閉渲染模式選擇器',
    currentMode: '目前模式',
    currentModeIs: '{mode} 是目前模式',
    currentModeActive: '{mode} 已啟用',
    renderAs: '將所選項目渲染為 {mode}',
    renderSelectionAs: '將此選擇渲染為 {mode}',
    outOfScope: '超出範圍',
    wontFix: '不修',
    toBacklog: '→ 待辦',
    deprecated: '已下線'
  },
  renderAsDock: {
    openRenderAs: '開啟渲染為 · {mode}',
    tapToHide: '點擊收起',
    tapToOpen: '{mode} · 點擊開啟渲染為',
    pickMode: '選擇模式'
  },
  stakeholders: {
    addressAria: '轉向 {name}',
    theStakeholders: '相關方',
    hideActions: '隱藏相關方操作',
    openStakeholders: '開啟相關方 · {name}',
    tapToHide: '點擊隱藏',
    tapToOpen: '{name} · 點擊開啟相關方',
    pickPersona: '選擇角色',
    personaMenu: '相關方角色',
    castGroup: '相關方陣容',
    castLabel: '相關方',
    castOneOfMany: '{name} 是 {count} 位相關方之一',
    castSpeaking: '{name} 正在發言',
    castAskCommentary: '請 {name} 點評',
    align: '給 CEO 準備稿',
    seniorDivider: '樓上',
    teamActionsHeading: '讓團隊動起來',
    delegateDivider: '把活交給…',
    delegateAria: '把活交給 {name} — 他們會{action}',
    handoffAck: '{name} 接下了',
    introEyebrow: '👥 圓桌會議已召開',
    introBody:
      '一位相關方正在對你的圖表發表意見——無論你是否詢問，他們都會在你工作時插話。可以從「相關方」把活交給其中任何一位，或者把整個團隊叫過來開個小會。想清靜就在工位上開啟專注時間。',
    introDismiss: '知道了',
    introAria: '認識相關方'
  },
  invite: {
    title: '接入外部智慧代理',
    subtitle: '邀請另一個 LLM 加入協同設計圓桌。',
    close: '關閉',
    explainer:
      '外部智慧代理透過 MCP 加入 — 它們可以查看圖表、提出修改，並與相關方一起發表意見。掃描 QR 碼或點擊立即連線，一鍵配對 IDE 端智慧代理；在任何人動圖表之前，你仍需核准握手。長期設定請使用進階選項中的穩定 URL。',
    loading: '正在載入邀請…',
    loadFailed: '載入邀請失敗。',
    rotateFailed: '輪換配對碼失敗。',
    scanToConnect: '掃碼連線',
    qrAlt: 'MCP 配對 URL 的 QR 碼',
    connectNow: '立即連線',
    connect: '連線',
    copy: '複製',
    copied: '已複製！',
    copyCode: '複製代碼',
    rotateCode: '輪換代碼',
    rotating: '輪換中…',
    advancedToggleShow: '顯示穩定 MCP URL 與舊版選項',
    advancedToggleHide: '隱藏穩定 MCP URL 與舊版選項',
    stableMcpUrl: '穩定 MCP URL（一次設定）',
    addCursorStable: '新增到 Cursor（穩定）',
    addToCursor: '新增到 Cursor',
    claudeCodeStable: 'Claude Code（穩定 URL）',
    sessionMcpLegacy: '工作階段 MCP URL（舊版）',
    pairingCode: '配對碼',
    pairingCodeAria: '配對碼'
  },
  insights: {
    paneLabel: '思考與分析',
    title: '思考',
    live: '進行中',
    empty: '智慧代理的思考與評審回覆會顯示在這裡。',
    tipLabel: '爛圖建築師小貼士™',
    statusIssue: '問題',
    statusStopped: '已停止',
    statusDone: '完成',
    statusWorking: '進行中',
    nowIssue: '問題',
    now: '目前',
    contentAnalysis: '分析',
    contentExplanation: '講解',
    contentRefinement: '精修',
    contentInnovation: '創新',
    contentRussMode: '三個逗號模式',
    contentUpdates: '內容更新',
    resultingInfographic: '產生的資訊圖',
    resultingChart: '產生的數據圖',
    resulting3d: '產生的 3D 場景',
    resultingPage: '產生的頁面',
    resultingForm: '產生的表單',
    resultingDiagram: '產生的架構圖',
    loadOntoCanvas: '將此圖表載入到畫布。',
    patchFromTool: '來自智慧代理工具的圖表修補',
    runDetails: '執行詳情',
    elapsedTime: '已用時間',
    totalTime: '總時間',
    critique: '評審',
    suggestion: '建議',
    note: '備註',
    levelLadder: '等級階梯',
    earnXp: '如何獲得經驗',
    trophyShelf: '獎盃架',
    closeLevelDetails: '關閉等級詳情',
    patchPreview: '修補預覽',
    sourceContext: '來源情境',
    jumpToVersion: '將畫布跳回圖表的此版本。',
    closeThinking: '關閉思考面板',
    actionableImprovements: '可執行的改進',
    restore: '還原',
    retry: '重試',
    retryQuality: '以高品質重試',
    noStructuralChanges: '此版本與該步驟之前的圖表相比未偵測到結構性變化。',
    stopRequest: '停止請求',
    hide: '隱藏',
    highlightOnCanvas: '在畫布上醒目顯示',
    clearHighlights: '清除畫布醒目顯示',
    highlightedChanges: '畫布上已醒目顯示的變化',
    removedFromDiagram: '已從圖表移除：{ids}',
    rawStreamEvents: '原始串流事件（{count}）',
    via: '來自',
    modeSuffix: '模式',
    brainSuffix: '模型',
    phaseStep: '階段 {step}',
    streakTitle: '{name} 連擊',
    diffAdded: '+{count} 新增',
    diffChanged: '~{count} 修改',
    diffRemoved: '−{count} 移除',
    changesSincePrevious: '相對上一版本的變化',
    showThinking: '顯示{thinking}',
    hideThinking: '隱藏{thinking}',
    showThinkingPanel: '顯示{thinking}面板',
    hideThinkingPanel: '隱藏{thinking}面板',
    streamDone: '完成',
    awaitingDecision: '等待你的決定。',
    attributedNoteStatus: '備註',
    errorPrefix: '錯誤',
    nowStatus: {
      repairingSyntax: '正在修復圖表語法…',
      stillWorking: '仍在處理…',
      thinking: '思考中…',
      applyingPatch: '正在套用圖表修補…',
      planningUpdate: '正在規劃更新…',
      polishing: '正在修好有問題的地方…',
      fixingDinesh: '正在修好別人都沒發現的地方…',
      restructuring: '正在重構圖表…',
      reviewing: '正在評審圖表…',
      explaining: '正在講解圖表…',
      goingOffScript: '正在放飛…',
      updatingStyle: '正在更新視覺樣式…',
      simplifyingBarker: '正在為董事會精簡…',
      workingOnDiagram: '正在處理圖表…',
      workingOnRequest: '正在處理你的請求…'
    },
    goIntent: {
      go: '開始',
      goDiagram: '開始 — 圖表',
      goQuoted: '開始「{excerpt}」',
      delegateGo: '{name} · {title}',
      batchMore: '（+{count} 項）',
      edge: '連線',
      node: '節點',
      subgraph: '子圖',
      item: '項',
      title: '標題',
      description: '描述',
      itemDesc: '項描述',
      itemValue: '項數值',
      itemIcon: '項圖示',
      mark: '標記',
      axis: '座標軸',
      legend: '圖例',
      element: '元素'
    },
    streamFailures: {
      staleRevision: '圖表已在別處變更 — 請重新整理後重試。',
      timeout: '執行逾時 — 請嘗試快速模式或重試。',
      network: '連線或串流逾時，請重試。',
      syntaxExhausted: '無法套用有效結果。',
      noPatch: '未套用圖表修補，請重試或嘗試高品質模式。',
      generic: '發生問題，你可以重試。'
    },
    syntaxFixer: {
      repaired: '已修復無效 DSL 並套用修補。',
      rejected: '語法修復器輸出未通過驗證。',
      failed: '語法修復器無法修復原始碼。'
    },
    selectionKinds: {
      label: '標籤',
      node: '節點',
      timeline: '時間軸',
      participant: '參與者',
      cluster: '子圖',
      edge: '連線',
      title: '標題',
      description: '描述',
      value: '數值',
      icon: '圖示',
      item: '項',
      mark: '標記',
      axis: '座標軸',
      legend: '圖例',
      element: '元素'
    }
  },
  editor: {
    doneEditing: '完成編輯',
    openEditor: '開啟程式碼編輯器',
    closeEditor: '關閉程式碼編輯器',
    codeTitle: '程式碼 · 編輯圖表原始碼',
    code: '程式碼',
    close: '關閉'
  },
  loading: {
    applyingChange: '正在套用圖表修改。',
    transforming: '正在變換圖表。',
    analyzing: '正在分析圖表。',
    applyingFixes: '正在套用評審修復。',
    applyingStyle: '正在套用樣式調整。',
    resetting: '正在重設圖表。',
    fixingPage: '正在修復頁面執行階段錯誤。',
    fixingMermaid: '正在修復 Mermaid 語法。',
    hydrating: '正在載入共享工作階段。',
    refreshing: '正在重新整理圖表。',
    working: '正在處理你的請求…',
    stopped: '已停止。',
    proposalApplied: '提案已套用。',
    proposalRejected: '提案已拒絕。',
    proposalStale: '提案已過期。',
    proposalResolved: '提案已處理。',
    invalidRoom: '房間碼無效或已過期。',
    simplifyFailed: '無法簡化講解。',
    micDenied: '語音辨識被拒絕麥克風權限。',
    voiceFailed: '語音輸入失敗，請重試。',
    voiceUnavailable: '此瀏覽器無法使用語音輸入。'
  },
  errors: {
    notifications: '錯誤通知',
    dismiss: '關閉錯誤',
    details: '錯誤詳情',
    chartFailed: '圖表無法渲染。',
    pageFailed: '頁面無法渲染。',
    formFailed: '此表單無法渲染。'
  },
  fullscreen: {
    enter: '進入全螢幕',
    exit: '退出全螢幕'
  },
  graphEdit: {
    connectHint: '點擊一個框以連結 · Esc 取消',
    undo: '復原',
    deleted: '已刪除',
    linked: '已連接',
    failed: '無法更新圖表',
    stale: '圖表已變化，請再試一次',
    renamePlaceholder: '標籤',
    nameNodeTitle: '命名此節點',
    nameNodeSkip: '保留預設',
    nameNodeConfirm: '命名'
  },
  advisorThinking: {
    gilfoyle: '不為所動',
    dinesh: '想要被認可',
    erlich: '正在推銷願景',
    russ: '在秀逗號',
    jared: '正在謹慎地提出',
    richard: '正在命名',
    default: '正在思考'
  },
  planBeat: {
    agent: '智慧代理',
    plan: '計畫',
    samePreviewAbove: '同上圖 ↑'
  },
  gamificationHud: {
    bonus: '獎勵',
    streak: '連擊',
    combo: '連擊',
    levelUp: '等級提升',
    lvlPrefix: '等級',
    max: '滿級',
    xpLabel: '經驗',
    tapForDetails: ' — 點擊查看詳情',
    levelAriaMax: '等級 {level}，已滿級，累計 {totalXp} {xpLabel}',
    levelAriaProgress: '等級 {level}，距下一級還需 {xpInto}/{xpForNext} {xpLabel}',
    levelFallbackTitle: '爛圖建築師'
  },
  brand: {
    totalSlopRuns: '累計 {count} 次爛圖執行',
    tapToShowXp: '點擊顯示經驗',
    tapToHideXp: '點擊隱藏經驗'
  },
  presence: {
    connected: '已連線的外部智慧代理',
    moreAgents: '另有 {count} 個智慧代理'
  },
  proposal: {
    loadPreviewTitle: '在主畫布上載入此提案以全尺寸預覽。不會接受該提案。',
    openFullPreview: '開啟全螢幕預覽',
    showSource: '顯示原始碼',
    proposedAria: '來自 {name} 的 {type} 編輯提案',
    proposedEdit: '提出了一個 {type} 編輯',
    reject: '拒絕',
    rejecting: '拒絕中…',
    accept: '接受並套用',
    applying: '套用中…',
    statusPrefix: '狀態：'
  },
  handshake: {
    title: '一個外部智慧代理想加入你的工作階段',
    reportedClient: '回報的用戶端：',
    explainer:
      '如果你允許，該智慧代理可以讀取你的圖表、提出修改（每一項都需你核准）、留下署名備註並對修訂作出反應。它無法直接套用修改。',
    deny: '拒絕',
    denying: '拒絕中…',
    allow: '允許智慧代理',
    allowing: '允許中…'
  },
  appError: {
    title: '出錯了',
    body: '應用遇到意外錯誤並停止渲染。重新載入以恢復。',
    reload: '重新載入應用'
  },
  checklist: {
    fixSelected: '修復所選',
    fixAll: '全部修復'
  },
  diagramCanvas: {
    streamingSource: '正在串流已驗證的原始碼…',
    done: '完成',
    loadingEditor: '正在載入程式碼編輯器…',
    selectAll: '全選',
    copy: '複製'
  },
  anythingCanvas: {
    canvas: '任意畫布（沙箱）',
    dismissError: '關閉執行階段錯誤'
  },
  embeddedPreview: {
    infographic: '資訊圖預覽（唯讀）',
    chart: '圖表預覽（唯讀）',
    metaphor3d: '3D 隱喻預覽（唯讀）',
    page: '頁面預覽（唯讀）',
    mermaid: 'Mermaid 架構圖預覽（唯讀）',
    forms: '表單預覽（唯讀）'
  },
  diagramSurface: {
    controls: '圖表介面控制項'
  },
  runFx: {
    reviewing: '評審中'
  },
  advisor: {
    pinned: '已釘選',
    suggestionNav: '相關方建議導覽',
    nextComment: '下一則相關方評論',
    drillDeeperAria: '深入挖掘 — 開啟完整的架構長篇論述',
    drillDeeperTitle: '在思考面板中開啟完整的架構深潛',
    pinTitle: '已釘選 — 點擊取消釘選',
    unpinTitle: '點擊釘選此評論',
    olderSuggestion: '較早的建議',
    olderSuggestionAt: '較早的建議（{pos}）',
    oldestSuggestion: '最早的建議',
    applySuggestion: '套用 {name} 的建議'
  },
  metaphor: {
    legend: '圖例',
    reading: '如何閱讀此場景',
    layers: '圖層',
    layer: '圖層',
    selected: '選取的元素',
    dismiss: '取消選取',
    type: '隱喻類型',
    viewAs: '檢視為',
    kinds: {
      city: '城市',
      layercake: '千層蛋糕',
      galaxy: '星系',
      tree: '樹',
      terrain: '地形',
      orrery: '天體儀',
      river: '河流',
      garden: '花園',
      archipelago: '群島',
      machine: '機械',
      bridge: '橋樑',
      cycle: '循環',
      subway: '地鐵',
      iceberg: '冰山',
      composite: '組合'
    },
    layerFocus: '單獨查看這一層，世界的其餘部分會後退',
    layerFocusClear: '恢復完整世界',
    layerAdd: '複製此圖層',
    layerAddFull: '最多四個圖層',
    layerRemove: '移除此圖層',
    layerRemoveLast: '組合場景至少需要一個圖層',
    compositeHintTitle: '組合模式',
    compositeHintBody: '融合世界正在等待至少一個語意層。請讓 Go 完成場景。',
    tour: {
      start: '導覽解說',
      startTitle: '逐步了解這個場景：它是什麼、如何閱讀、重點在哪裡',
      aria: '本場景的導覽解說',
      step: '第 {step} 步，共 {total} 步',
      close: '結束導覽',
      back: '上一步',
      next: '下一步',
      done: '完成',
      overview: '場景概覽',
      overviewBody: '拖曳可環繞檢視。每一步都會拉近到正在解說的部分。',
      legend: '如何閱讀',
      peak: '最突出的元素',
      peakBody: '{axis}最大：{value}。',
      layer: '圖層',
      layerBody: '以{kind}呈現，共 {count} 個元素。',
      layerBodyWithPeak: '以{kind}呈現，共 {count} 個元素——最大的是{item}。',
      link: '彼此如何連接',
      linkBody: '{from} → {to}',
      accent: '重點'
    }
  },
  styleEdits: {
    region: '樣式編輯',
    title: '視覺微調',
    iconReplace: '替換圖示',
    colorShift: '色彩變更',
    apply: '套用樣式微調'
  },
  runTimeline: {
    summary: '執行摘要',
    activity: '執行活動時間軸',
    runActivity: '執行活動',
    intent: '意圖',
    triggeredBy: '觸發來源',
    validationFeedback: '驗證回饋',
    elapsedSoFar: '已用時',
    stepDuration: '步驟耗時',
    elapsedRunTime: '已執行時間',
    totalRunTime: '總執行時間',
    runCostEstimateLabel: '預估執行成本',
    runCostTotalLabel: '執行合計',
    runCostEstimateTitle: '本次智慧體執行的近似 LLM 花費（非帳單金額）',
    timeInStepSoFar: '此步驟已用時',
    timeSpentInStep: '此步驟耗時',
    endedWithIssue: '以問題結束',
    stopped: '已停止',
    done: '完成',
    phases: {
      run_started: '開始',
      planning: '規劃',
      analyze: '分析',
      analyze_stream: '串流',
      intent: '套用',
      agent_run: '工具',
      transform: '變換',
      syntax_fixer: '語法',
      chart_syntax_fixer: '語法',
      metaphor_syntax_fixer: '語法',
      anything_syntax_fixer: '語法',
      syntax_repair: '修復',
      patch_retry: '重試',
      invoke: '生成',
      invoke_fallback: '收尾',
      repair_1: '修復',
      repair_2: '修復',
      chart_transform: '變換',
      chart_style: '樣式',
      chart_analyze: '分析',
      chart_invoke: '生成',
      metaphor_transform: '變換',
      metaphor_analyze: '分析',
      metaphor_invoke: '生成',
      anything_transform: '變換',
      anything_analyze: '分析',
      anything_invoke: '生成',
      forms_transform: '變換',
      forms_analyze: '分析',
      forms_invoke: '生成',
      activity: '活動'
    },
    running: {
      model: '推理中…',
      fixer: '修復中',
      patch: '驗證中',
      inspect: '讀取上下文',
      tool: '處理中'
    },
    doneLabels: {
      model: '本輪完成',
      fixer: '修復完成',
      patch: '更新已接受',
      inspect: '上下文已載入',
      tool: '完成'
    },
    validationFailed: '驗證失敗',
    interrupted: '已中斷',
    queued: '排隊中',
    executionMode: {
      llm: 'LLM',
      code: '程式'
    },
    kicker: {
      live: '即時活動',
      issue: '執行問題',
      stopped: '執行已停止',
      activity: '執行摘要'
    },
    headline: {
      working: '處理中…',
      stoppedOnIssue: '因問題停止',
      stoppedByYou: '被你停止',
      recovered: '已恢復並完成',
      allComplete: '所有步驟已完成'
    },
    units: {
      phase: '個階段',
      phases: '個階段',
      modelTurn: '輪模型',
      modelTurns: '輪模型',
      toolRun: '次工具呼叫',
      toolRuns: '次工具呼叫',
      planBeat: '個計劃節拍',
      planBeats: '個計劃節拍',
      repair: '次修復',
      repairs: '次修復',
      issue: '個問題',
      issues: '個問題',
      technicalStep: '個技術步驟',
      technicalSteps: '個技術步驟'
    },
    patchLines: '+{added} / −{removed} 行'
  }
};
