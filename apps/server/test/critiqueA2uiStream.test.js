...
test('emitCritiqueA2uiBeforeFinal emits a2ui when jared analyze has actionable items', () => {
  const out = [];
  const emit = (evt) => out.push(evt);
  const md = `## Actionable improvements\n\n- One\n`;
  emitCritiqueA2uiBeforeFinal(emit, { kind: 'jared', analyzeText: md });
...
test('emitCritiqueA2uiBeforeFinal skips non-jared analyze kinds', () => {
  ...
  emitCritiqueA2uiBeforeFinal((e) => out.push(e), {
    kind: 'critique',
    analyzeText: '## Actionable improvements\n\n- One\n'
  });
  assert.deepEqual(out, []);
});
