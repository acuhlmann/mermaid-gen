...
test('resolveAgentRecursionLimit defaults and clamps', () => {
  ...
  assert.equal(resolveAgentRecursionLimit({ MERMAID_AGENT_RECURSION_LIMIT: '' }), DEFAULT_MERMAID_AGENT_RECURSION_LIMIT);
  assert.equal(resolveAgentRecursionLimit({ MERMAID_AGENT_RECURSION_LIMIT: 'not-a-number' }), DEFAULT_MERMAID_AGENT_RECURSION_LIMIT);
...
test('getAgentRunnableConfig exposes recursionLimit', () => {
  assert.deepEqual(getAgentRunnableConfig({ MERMAID_AGENT_RECURSION_LIMIT: '42' }), { recursionLimit: 42 });
});
...
test('resolveAgentToolCallRunLimit defaults disables at zero and clamps', () => {
  ...
  assert.equal(resolveAgentToolCallRunLimit({ MERMAID_AGENT_MAX_TOOL_CALLS_PER_RUN: '' }), DEFAULT_MERMAID_AGENT_MAX_TOOL_CALLS_PER_RUN);
  assert.equal(
...
test('createDiagramAgentMiddleware is empty when tool run limit disabled', () => {
  assert.equal(createDiagramAgentMiddleware({ MERMAID_AGENT_MAX_TOOL_CALLS_PER_RUN: '0' }).length, 0);
});

test('createDiagramAgentMiddleware returns one limiter when enabled', () => {
  assert.equal(createDiagramAgentMiddleware({ MERMAID_AGENT_MAX_TOOL_CALLS_PER_RUN: '8' }).length, 1);
});
