...
/** @type {Awaited<ReturnType<typeof attachFullApp>> | null} */
let exportsCache = null;

/**
 * Wire the full Express app onto the early-listen host. Called after the HTTP
 * port is open so Cloud Run startup probes can succeed while heavy modules load.
 *
 * @param {import('express').Express} app
 * @param {import('node:http').Server} server
 * @param {(handler: import('express').RequestHandler) => void} registerHealthHandler
 */
export async function attachFullApp(app, server, registerHealthHandler) {
  ...
  registerHealthHandler((_req, res) => {
...
export function getServerExports() {
  return exportsCache;
}
