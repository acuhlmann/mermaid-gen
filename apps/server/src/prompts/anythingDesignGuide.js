...
- Canvas 2D methods take positional numeric args, never a single rect-shaped object: ctx.fillRect({x, y, width, height}) throws "4 arguments required, but only 1 present". If a helper returns a {x, y, width, height} shape, spread it yourself — ctx.fillRect(r.x, r.y, r.width, r.height). Same for strokeRect, clearRect, and arc's separate x/y/radius/startAngle/endAngle args.
...
- Declare const/let before first use in the same script — node.attr(...), arrowEls.forEach(...), or any read of a binding on a line above its declaration throws a TDZ ReferenceError at runtime. This includes referencing the variable inside its own initializer: const body = Bodies.rectangle(x, y, w, h, { id: body && body.id }) throws "Cannot access 'body' before initialization" on the very first call, because body is still being assigned when the object literal that reads it evaluates. If you need a previous value, read it from a variable declared earlier, never from the one the current statement is creating.
...
- With @lib:matter, build every body (ground, walls, boxes) before any function that repositions, removes, or reads them — including a resize handler called once up front, or a "rebuild by removing then re-adding" function meant to run on every call after the first. Guarding on the Engine existing, or on a comment like "if bounds exist already, remove first", is not enough: Matter.Body.setPosition(ground, {x, y}) throws "Cannot read properties of undefined (reading 'position')" and Matter.Composite.remove(world, ground) throws "Cannot read properties of undefined (reading 'type')" when ground is still the undefined a bare "let ground;" declaration leaves it as. Call the body-creation function first, then resize/layout/rebuild — never the reverse — or guard the remove call itself with a null check (if (ground) Composite.remove(world, ground);).
- With @lib:matter, measure the container's real size (a getBoundingClientRect() read) before creating any frame body — floor, walls — whose width, height, or position is computed from it. Building the frame while those variables still hold their placeholder 0 bakes a zero-area shape into the world; a later resize handler that only calls Body.setPosition moves that degenerate body without ever resizing it, since a body's shape is fixed at creation (see above). Colliding a real box against a zero-width floor or wall can drive the physics to NaN/Infinity, which then throws far from its real cause — e.g. ctx.createLinearGradient(bounds.min.x, ...) reporting "The provided double value is non-finite" when it later reads that body's bounds. Measure first, then build the frame from real numbers.
- With @lib:matter, a body's shape is fixed at creation — there is no Matter.Body.setDimensions(...) or Matter.Body.setLengths(...) to resize one later. To resize ground/walls on layout or window resize, either Matter.Body.scale(body, scaleX, scaleY) the existing body, or Matter.Composite.remove the old one and add a freshly-created replacement; either invented method throws "is not a function" the same way setStatic/setPosition typos do.
- With @lib:matter, never pass render: false in a body's option object to hide it — Bodies.rectangle/circle merges the render option with Matter's defaults (including render.sprite = {xOffset, yOffset, ...}), and render: false skips that merge entirely instead of disabling it, leaving body.render falsy. A later internal Matter read of body.render.sprite.xOffset (during a position or vertex update) then throws "Cannot read properties of undefined (reading 'xOffset')". To hide a body from Matter's own renderer, set render: { visible: false } — or omit render entirely.
- With @lib:matter, the only step calls are Matter.Runner.run(runner, engine) and Matter.Runner.tick(runner, engine, time) — there is no Matter.Runner.update(...). And once Matter.Mouse.create(canvas) is wired into a MouseConstraint, mouse tracking is automatic; there is no Matter.Mouse.update(...) or Matter.Mouse.setOffsetSafe(...) to call per frame — both throw "is not a function". The Mouse object itself has no .on() either — mouse.on(mouse, 'mousedown', fn) or Matter.Mouse.on(mouse, ...) throws "Mouse.on is not a function". Mouse press/drag events fire on the MouseConstraint once created: Matter.Events.on(mouseConstraint, 'mousedown', fn), 'mouseup', 'mousemove', 'startdrag', and 'enddrag' are the real event names — listen on mouseConstraint, never on mouse.
- With @lib:matter, a body-factory function must return what it builds — function buildFrame() { Bodies.rectangle(...); Bodies.rectangle(...); Bodies.rectangle(...); } creates three bodies but hands the caller nothing back, so const walls = buildFrame(); Composite.add(world, walls); passes undefined to Composite.add, which throws "Cannot read properties of undefined (reading 'type')" reading obj.type off the missing object. Collect each Bodies.* call into an array (or object) and return it, or call Composite.add(world, [...]) directly inside the factory before returning.
