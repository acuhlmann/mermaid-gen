...
- With @lib:matter, the only step calls are Matter.Runner.run(runner, engine) and Matter.Runner.tick(runner, engine, time) — there is no Matter.Runner.update(...). And once Matter.Mouse.create(canvas) is wired into a MouseConstraint, mouse tracking is automatic; there is no Matter.Mouse.update(...) or Matter.Mouse.setOffsetSafe(...) to call per frame — both throw "is not a function".
