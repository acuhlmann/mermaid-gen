...
- "composite" — describe 1–4 semantic \`layers\` that the renderer fuses into **one integrated kinetic world**. Each layer has \`as\` = a base kind and its own \`items\` + \`label\`, but it is NOT a montage of complete scenes. Use \`layout: "fused"\` for all new composites. \`adjacent\` and \`overlay\` are legacy compatibility layouts and must not be emitted for new work.
  ...
  The generic planner attaches landmarks to shared sites, routes paths through them, places fields and accents around real anchors, and draws cross-item links between those anchors. Align grouping nouns across layers so the world coheres: reuse the same \`district\` / \`chain\` / \`bed\` strings (or matching labels) when a landmark belongs on a particular island or bed — the fused planner binds by those affinities instead of random site attachment, AND draws each shared noun as a named zone on the floor of the world. Two layers that agree on "Checkout" therefore produce one labelled Checkout territory containing both; two that disagree produce two anonymous halves. This is the highest-leverage thing you control in a composite, so spell the grouping nouns identically across layers and take them from the user's own vocabulary. Carry storytelling fields into composite layers too (\`hazard\`, \`health\`, \`lighting\`, \`condition\`, \`maturity\`, \`cracks\`, \`tilt\`); the kinetic renderer reads them. Each layer tells a different slice of the same topic: do not clone the same actor into every layer. Preserve exact user nouns in visible labels. Cross-layer \`links\` may connect globally unique item ids on different layers. Keep top-level \`items\` empty. Prefer 2–3 layers; use 1 or 4 when the subject genuinely calls for it.

  Layer \`label\` is a reading key, not a restatement of \`as\`. Write it so a viewer who cannot read 3D still knows the slice: "Commerce domains as islands", "Services as towers", "Order journey as a river" — never "archipelago" or "Layer 1". The renderer prints these labels as an on-canvas legend.
  ...
  Example:
{
  ...
  "layers": [
    { "id": "domains", "as": "archipelago", "label": "Commerce domains as islands", "items": [{ "id": "checkout", "label": "Checkout", "mass": 12, "relief": 0.8, "chain": "Buy", "accent": true, "note": "Checkout is the load-bearing domain — payments and the order river both sit on it", "glyph": "money" }] },
    { "id": "services", "as": "city", "label": "Services as towers", "items": [{ "id": "payments-api", "label": "Payments API", "height": 14, "footprint": 3, "district": "Checkout", "lighting": "lit", "glyph": "gateway" }] },
    { "id": "journey", "as": "river", "label": "Order journey as a river", "items": [{ "id": "place-order", "label": "Place order", "stage": 0, "flow": 10, "glyph": "user" }] }
  ],
  "items": [],
...
- **Every link is drawn as an arrow, and it points at \`to\`.** Order the pair by the sentence you mean: flow goes source → destination, a dependency goes from the dependent to the thing it needs, ownership goes from the owner to the thing owned. Swapping them draws the opposite claim, so a reversed pair is a factual error rather than a style choice. A short \`label\` (a verb — "verifies", "settles nightly", "owns") is what turns an arrow into a sentence; give one to any link whose direction alone does not say why it exists.
...
2. Name groupings from the user's domain. Districts, clusters, beds, chains, axles, and layer labels must come from the topic ("checkout", "ml-platform", "EU region") — never generic filler ("group 1", "misc"). Composite layer labels must also name the spatial grammar: "X as islands", "Y as towers", "Z as a river".
...
The renderer draws an always-on title card (\`scene.title\` + \`scene.subtitle\`) and a legend panel (\`scene.legend.<axis>\`) over the canvas, a thesis line from the accented item's \`note\`, and a hover tooltip that shows each item's label next to its encoded metric. These fields are the difference between a pretty-but-opaque scene and a self-explanatory one — never leave them empty.

For every metaphor you MUST:
- Set \`scene.title\` (the subject in a few words) and \`scene.subtitle\` (one-line context that also names the insight: what the extreme element is claiming, plus date/scope). "Checkout carries the load, Aug 2026" beats "Production stack, Aug 2026".
...
- Set \`accent: true\` plus a claim-shaped \`note\` on the headline item. The overlay repeats that note as the scene's thesis.
...
- Composite: set legend axes for every encoding used across layers (mass, height, flow, …). Each layer \`label\` is a reading key that names the slice AND the grammar ("Domains as islands"), never just the kind name.
