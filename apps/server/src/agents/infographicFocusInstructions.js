...
export function buildInfographicAnalyzeFocusInstructions(focusNode, kind) {
  ...
  // critique
  if (role === 'title' || role === 'top-level description') {
    return `\n\nSelection focus: The user selected ${summary} In ## Weaknesses and limits, ## Template fit, and ## Actionable improvements, prioritize how well this ${role} frames the body and whether it could be sharper, more specific, or better aligned with the data items below. Keep ## Strengths and ## Visual and information density but tie them to how this ${role} reads in context.`;
  }
  return `\n\nSelection focus: The user selected ${summary} In ## Weaknesses and limits, ## Template fit, and ## Actionable improvements, prioritize issues with this specific item — its label clarity, value accuracy, icon fit, position in the sequence/list, and whether it deserves to stay or be merged/replaced. Keep ## Strengths and ## Visual and information density but reference this item where they apply. Address sibling items only after covering ${target}.`;
}
