...
**Transform agent.** Same LangChain instance, same tools, but the user message is generated by `buildTransformUserContent` for one of `refine` | `erlich` | `goMad`. Sampling and prompt budgets vary per mode; **Go Mad** also takes a _depth_ parameter (`goMadStreak + 1`, capped at 12) that escalates temperature.
