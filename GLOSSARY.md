...
**Cast tiers (team / senior / office).** The three-tier org chart of the fiction (`apps/web/src/utils/castTiers.js`). **Team** = the five advisor personas you work with daily (refine, erlich, goMad, critique, explain) — the proactive roundtable and the agent actions. **Senior** = the executives your team presents to (`barker` Jack Barker, `ciso` Sasha, `cto` Marcus, `cfo` Diane): steering meetings and at most one high-stakes email per session, never ambient pings. Barker additionally holds the sixth radial advisor seat (inherited from the retired `exec` — see `docs/recipes/replicate-tv-character.md`), summoned only, never in the roundtable. **Office** = the ambient floor (Chad, Pam, Dave, Gary, Linda, Ulrich). Tier is a tag, not a data move — `barker` lives in both `VARIANT_PERSONAS` and `SENIOR_STAKEHOLDERS`, `ciso` in `OFFICE_COLLEAGUES`.
