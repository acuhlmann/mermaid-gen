...
**Canvas graph edit.** Point-and-click Add / Delete / Rename / Link on a rendered graph (`origin: user` via `POST /api/copilotkit/user-edit`). Not a second editor — a mutator rewrites the slot DSL. Shipped: Mermaid flowchart, mindmap, stateDiagram-v2, and sequenceDiagram; Infographic hierarchy-tree/mindmap, relation-dagre, relation-network, lists/sequences; Metaphor3D tree, city, and garden; Chart inline `data.values` rows. Family table: `docs/canvas-graph-edit.md`.
