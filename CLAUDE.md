...
**Mermaid is on 12.x.** Four places declare its version — three `package.json`s plus a `mermaid@<major>` CDN URL in `mcpAppDiagramPreview.js` that npm never sees — and `npm run verify:mermaid-pins` proves they agree. A new type is a six-registry edit: the table, including the two v12 types' per-directive styling gaps (agentflow rejects `:::`, usecase rejects `linkStyle` — so neither belongs in `STYLE_DIRECTIVE_UNSUPPORTED_TYPES`), is in [`docs/agent-blast-radius.md`](docs/agent-blast-radius.md).

Two things that cost an hour each to rediscover: v12's `-beta` suffix **is** the keyword, so `agentflow TD` parses under nothing (`normalizeDiagramHeader` promotes it); and of v12's three default flips only `layout` reaches us — `theme`/`look` were already pinned by `DEFAULT_DIAGRAM_STYLE_VALUES`, so ELK silently replaced Dagre, with `DiagramStyleSchema.layout` as the deliberately-defaultless lever. Do not try to dedupe `streamdown`'s nested `mermaid@11`: both `overrides` forms leave it on disk and turn `npm ls` red. Measure geometry in a real browser — jsdom's stubbed `getBBox` reported a 204704-wide viewBox where Chromium gave 360.
...
- `apps/server/src/mcp/apps/mcpAppSessionBridge.js` — the MCP Apps postMessage/SSE protocol every `*AppHtml.js` string talks to; a change here can strand external agents silently with no failing test. The App HTML is **not** off-limits — `mcpAppHtml.test.js` pins it and `npm run verify:mermaid-pins` guards its Mermaid CDN major.
