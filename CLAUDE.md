...
**Anything** (shape + policy + quality lint + runtime check + fixer + agent repair, no sanitizer): `parseAnythingHtml` → `lintAnythingPolicy` → `lintAnythingQuality` in `packages/shared` → `runAnythingRuntimeCheck` (`apps/server/src/tools/anythingRuntimeCheck.js` — executes page JS in an isolated jsdom child process emulating the iframe sandbox; rejects uncaught errors, hangs, and blank renders; agent patches only, client sync skips it; kill switch `ANYTHING_RUNTIME_CHECK=0`) → single-shot `anythingSyntaxFixer.js` → agent repair turns (bounded by `ANYTHING_REPAIR_MAX_ATTEMPTS`). There is deliberately no HTML sanitizer — safety comes from the client rendering the slot in an `allow-scripts`-only sandboxed iframe with CSP (`AnythingRenderer.jsx`; never add `allow-same-origin`). On the client, `wrapAnythingSrcDoc` injects a post-validation runtime-error bridge (postMessage) so `AnythingRenderer` can surface in-iframe errors and feed load-phase failures into the auto-fix flow.
