...
**Mermaid is on 12.x. A new diagram type is a six-registry edit, not a one-line allowlist.** `agentflow-beta` and `usecase-beta` landed in 12.0.0, and to pass the gate a type must appear in: `DIAGRAM_PREFIX_PATTERN` (`apps/server/src/agents/mermaidReliabilitySkill.js`), `KNOWN_DIAGRAM_PREFIXES` and (only if it truly cannot take styling) `STYLE_DIRECTIVE_UNSUPPORTED_TYPES` (`packages/shared/src/mermaidSanitizer.ts`), `CANONICAL_TYPES` (`apps/server/src/agents/inferDiagramType.ts`), and `RULE_PACKS` (`apps/server/src/prompts/mermaidSyntaxGuard.js`) — plus `apps/server/src/mcp/apps/mcpAppDiagramPreview.js`'s CDN major, which is the same library loaded over the network for external agents and silently renders v11 grammar while the gate accepts v12. Canvas _editing_ is a further, larger step: `DIAGRAM_DIRECTIVES` (`mermaidSourceLocate.js`) + a `canvasGraphEdit.js` adapter + a `GRAPH_EDIT_BLAST_TESTS` entry, and an unlisted type degrades to `unknown` rather than breaking. Verify a new type with `node apps/server/scripts/benchMermaid.js --tag <label>` — it replays the corpus through the real ladder and exits non-zero on `expectationMatch` drift; read that, never `acceptRate`.

**The `-beta` suffix is part of the keyword, so the natural name is a guaranteed model failure** — `agentflow TD` parses under nothing. `normalizeDiagramHeader` promotes the bare name rather than spending a repair turn, and `inferDiagramType` resolves both spellings so the right rule pack is chosen _before_ the sanitizer runs. Same reasoning for why each new type is **not** in `STYLE_DIRECTIVE_UNSUPPORTED_TYPES`: that fixer strips all four styling lines, but the real gaps are per-directive and opposite — measured against `mermaid.parse`, **agentflow accepts `classDef`/`class`/`style`/`linkStyle` but rejects the `:::` suffix; usecase accepts `classDef`/`class`/`:::`/`style` but rejects `linkStyle`**. Blunt stripping would delete legitimate styling and hide the actual mistake, so those two facts live in the rule packs instead.

**Of 12.0.0's three default changes, only one reaches this repo.** It flips `theme` → `redux-color`, `look` → `neo`, and `layout` → `elk` (ELK is newly bundled). The first two are already neutralized: `DEFAULT_DIAGRAM_STYLE_VALUES` pins `theme: 'base'` / `look: 'neo'` and every diagram gets a `%%{init}%%` directive, so those were v11 values here already. **ELK was not pinned and is the one real behavioral change** — so `layout` is now a first-class key in `DiagramStyleSchema` (`packages/shared/src/mermaidStyle.ts`), deliberately left without a default because absence is what lets Mermaid choose, and validated as a _shape_ rather than an enum: layout loaders are pluggable and a membership list goes stale on a minor bump. Measured in Chromium on one flowchart, dagre 370×382 vs elk 360×402 — the lever works and neither is pathological. Two traps: the removed `defaultRenderer` key and the removed `createCommonLayoutRenderer`/`paintLayoutData`/`clearLayoutRenderState`/`defaultMeasureLayout` exports are gone from the public API (unused here), and **jsdom is useless for geometry** — a stubbed `getBBox` reported a 204704-wide viewBox for v12 and 43096 for v11 on the identical diagram; only a real browser gives numbers you can act on.

**CopilotKit still drags in a second Mermaid, and npm cannot dedupe it.** `@copilotkit/react-core` → `streamdown` declares `mermaid: ^11.11.0` as a hard (non-peer) dependency, so it keeps its own nested 11.x. Root `overrides` — both the global `"mermaid"` form and the parent-scoped `streamdown > mermaid` form — rewrite streamdown's resolved range to `^12.0.0` but leave the nested 11.x on disk, which makes the tree report `ELSPROBLEMS` / `invalid` from `npm ls`. **An override that npm cannot honor is worse than none**: it records an intent the repo cannot keep and turns a clean install red. Leave the duplicate and expect `npm ls mermaid` to show two copies; the app's own surfaces (canvas, previews, server gate) all use the hoisted 12.x.
...
| Routine budget + queue health          | `npm run routine:guard -- --postflight <name>`; `--filings [--json]` = backlog, net inflow, filings vs `maxIssues` (rule 12)                                               |
...
- `apps/web/src/ArchiSlop.jsx` (~1036 LOC; entry split to `App.jsx` + feature hooks under `hooks/`, `features/insights/*`, `features/streaming/*`, `features/canvas/*`, `features/session/*`, `features/prompt/*`, `features/shell/*`, `features/ceremony/*`, `features/advisor/*`, `features/desk/*`, `components/buildRadialActions` — see [`docs/decisions/0005-monolith-splits.md`](docs/decisions/0005-monolith-splits.md)), `apps/server/src/mcp/mcpServer.js` (~1406; helpers + first `tools/register*` modules extracted — continue per-tool splits), `apps/server/src/agents/mermaidLangChainAgent.js` (~1156), `apps/web/src/components/InsightsPane.jsx` (~1835), `apps/web/src/components/DiagramCanvas.jsx` (~1864; hit-test/connect-source resolvers for flowchart, sequence, infographic and mindmap graph-edit families extracted to `apps/web/src/utils/diagramGraphEditNodeResolve.js` — closes [issue #363](https://github.com/acuhlmann/mermaid-gen/issues/363)), `apps/web/src/components/RadialActionMenu.jsx` (~1164), `apps/server/src/agents/infographicLangChainAgent.js` (~846), `apps/server/src/routes/copilot.ts` (~1194; the user-edit route handler extracted to `apps/server/src/routes/copilotUserEdit.ts` — closes [issue #381](https://github.com/acuhlmann/mermaid-gen/issues/381)), `apps/web/src/state/diagramStore.js` (~603). Future per-tool splits in `mcpServer.js` should follow the same pattern: extract closure helpers into a sibling module and add a `register{ToolName}(server, ctx)` file under `apps/server/src/mcp/tools/`.
...
- **There is a night ladder, and it is a dependency order.** Eight jobs run between `0 15` and `45 0`
  ...
  each rung and each one's measured worst-case end) in [`docs/routines/review.md`](docs/routines/review.md). `deps`
  (`30 4,14 * * *`) is off the ladder on purpose — advisories arrive in bursts, and its night firing
  lands half an hour _before_ the window opens so a Dependabot merge never moves the base commit out
  from under a running job. `npm run verify:agent-infra` fails if a rung leaves the 15:00–01:00 UTC
  window, is missing from the table, or if a mirror table disagrees with a playbook's `schedule:`.
