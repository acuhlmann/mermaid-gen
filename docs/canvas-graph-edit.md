...
| Infographic chart values      | infographic  | `chart-*`                                               | `data-indexes` on `values` (`"0"`…`"n-1"`)         | sibling of selected                    | item; **last value refused**                  | item `label` | n/a                                                | **shipped** |
| Infographic flat hierarchy    | infographic  | `hierarchy-structure`                                   | `data-indexes` on `items` (`"0"`…`"n-1"`)          | sibling of selected                    | item; **last item refused**                   | item `label` | n/a                                                | **shipped** |
| Infographic compare           | infographic  | `compare-*` (SWOT, quadrant, binary)                    | AntV `data-indexes` on `compares`                  | child of selected compare root         | node + descendants; **last root refused**     | item `label` | n/a                                                | **shipped** |
| Mermaid mindmap               | mermaid      | `mindmap` header                                        | indent path                                        | child                                  | node + descendants                            | label        | n/a                                                | **shipped** |
| Mermaid class                 | mermaid      | `classDiagram`                                          | class name                                         | new class + association                | class + incident relations                    | class name   | association (`-->` default)                        | **shipped** |
| Mermaid ER                    | mermaid      | `erDiagram`                                             | entity name                                        | new entity + relationship              | entity + attribute block + incident relations | entity name  | relationship (default `\|\|--o{` cardinality)      | **shipped** |
...
| Mermaid timeline              | mermaid      | `timeline`                                              | flat event index (`"0"`…`"n-1"`) or `~label:`      | sibling of selected                    | event; **last event refused**                 | event label  | n/a                                                | **shipped** |
| Mermaid pie                   | mermaid      | `pie` / `pie showData`                                  | slice index (`"0"`…`"n-1"`) or `~label:`           | sibling of selected                    | slice; **last slice refused**                 | slice label  | n/a                                                | **shipped** |
...
7. **Mermaid classDiagram** — shipped (`mermaidClassEdit.js`). Classes + UML relations; Add declares a new class and an association from the source; Link appends `-->` when no relation exists. Rename rewrites the class identifier everywhere (relations, members, blocks). Parallel associations share `pickParallelEdgeRef` for delete. `canLink: true`.
8. **Mermaid erDiagram** — shipped (`mermaidErEdit.js`). Entities + cardinality relationships; Add declares a new entity (attribute stub) and a `||--o{` relationship from the source; Link appends the default cardinality when no matching relation exists. Rename rewrites the entity identifier everywhere (relations and `{ … }` blocks). Parallel relationships share `pickParallelEdgeRef` for delete. `canLink: true`.
9. **Metaphor3D tree** — shipped. JSON `parent` field; hit identity is item `id` via the Metaphor3D selection bridge (`MetaphorGraphEditBridge`). `canLink: false`.
10. **Metaphor3D city** — shipped. Flat `items[]` with `height`/`footprint`/`district`; Add inserts a sibling after the selection and clones the source building's encoding defaults. Delete refuses the last building (`reason: 'last'`). Link appends to `links[]`. Hit identity is item `id` via `MetaphorGraphEditBridge` (all metaphor kinds). `canLink: true`.
11. **Metaphor3D garden** — shipped. Flat `items[]` with `maturity`/`impact`/`health`/`bed`; Add inserts a sibling and clones the source plant's defaults. Delete refuses the last plant. `canLink: false`.
12. **Chart values** — shipped. Inline `spec.data.values` rows only (not `data.url`). Add inserts a sibling row after the selected datum index; Delete refuses the last row (`reason: 'last'`); Rename updates the primary nominal/string field for that row. Hit identity is Vega's datum index on `chart-mark` selections. `canLink: false`.
13. **Metaphor3D flat kinds** — shipped (`metaphorFlatKindEdit.js`). Layercake, galaxy, machine, and terrain share city-style sibling Add + `links[]` Link. Orrery, river, archipelago, bridge, cycle, subway, and iceberg share garden-style sibling Add with Link off — their ordering/grouping lives in item fields (`stage`, `phase`, `span`, `chain`, `berg`, `moon`, `line`/`interchange`), not free edges. All refuse deleting the last item (`reason: 'last'`). Hit identity is item `id` via `MetaphorGraphEditBridge` for every base kind.
14. **Metaphor3D composite** — shipped (`metaphorCompositeEdit.js`). Fused multi-layer worlds delegate Add/Delete/Rename to each layer's `as` kind on a mini-document, then merge items back. Item ids must stay globally unique across layers (Add re-ids on collision). Link writes the composite's top-level `links[]` and works cross-layer when at least two items exist. `renameCompositeEdge` sets or clears the matching top-level `{from,to}` link's `label` via the shared `renameLinkedEdge` helper (`reason: 'missing'` when no such link exists) — a top-level operation, not delegated to a layer's `as` kind. Hit identity is item `id` via `MetaphorGraphEditBridge`; the selection store carries the layer's `as` kind, not `"composite"`.
15. **Infographic chart + hierarchy-structure** — shipped (extends `infographicGraphEdit.js`). Flat `values` on `chart-*` (Add clones `value 0`; wordcloud uses `text`/`weight`) and flat `items` on `hierarchy-structure` share list-style Add/Delete/Rename with AntV `data-indexes`. `canLink: false`.
16. **Mermaid timeline** — shipped (`mermaidTimelineEdit.js`). Flat `section` / `Label : tail` events in source order; Add inserts a sibling after the selection and clones the source event's tail segment. Delete refuses the last event (`reason: 'last'`). Hit identity is flat index (`"0"`…`"n-1"`) or `~label:`; canvas resolves `g.timeline-node` clicks. `canLink: false`.
17. **Mermaid pie** — shipped (`mermaidPieEdit.js`). Flat `"Label" : value` slices; Add inserts a sibling after the selection and clones the source slice's value. Delete refuses the last slice (`reason: 'last'`). Hit identity is slice index or `~label:`. `canLink: false`.
18. **Infographic compare** — shipped (extends `infographicGraphEdit.js`). `compare-*` templates with a `compares` block (SWOT, quadrant, binary cards); Add appends a child under the selected compare root; Delete refuses the last top-level root (`reason: 'last'`). Same AntV `data-indexes` identity as hierarchy. `canLink: false`.
...
- **Metaphor3D city and garden identity is item `id`.** Same `MetaphorGraphEditBridge` as tree; `graphEditAdapterFor` sniffs `metaphor` on the JSON. City Add clones `height`/`footprint`/`district` from the selected building; garden Add clones `maturity`/`impact`/`health`/`bed`. Both refuse deleting the last item (`reason: 'last'`). City Link writes `links[]`; garden keeps Link off. After Add, `useFlowchartGraphEdit` reads `metaphor` back from the mutated source so the post-add descriptor is not stuck on `"tree"`. `renameCityEdge` sets or clears `MetaphorLinkSchema`'s optional `link.label` on the matching `{from,to}` pair (`reason: 'missing'` when no such link exists); there is no parallel-edge disambiguation to do here — `connectCityNodes` already refuses a duplicate `{from,to}` pair, so at most one link matches. **`renameCityEdge` and `deleteCityEdge` are registered on the adapter but unreachable from the canvas today** — `MetaphorGraphEditBridge` only ever emits `kind: 'metaphor-item'` descriptors, never `kind: 'edge'`, so `openRename`'s edge session (and the equivalent delete path) in `useFlowchartGraphEdit.js` never opens for a city link; a 3D link-hit descriptor is the missing piece, tracked in [#495](https://github.com/acuhlmann/mermaid-gen/issues/495) (#463 was closed by documenting this gap rather than fixing it — do not repoint back to it). Garden and chart still stub `renameEdge` as `fail('not-graph')`; garden has no free edge to rename in the first place. Composite now implements `renameCompositeEdge` on its top-level `links[]` the same way (`reason: 'missing'` when no such link exists) — see the composite paragraph below. Layercake/galaxy/machine/terrain (`canLink: true`, same `links[]` shape) now implement `renameEdge` the same way as city — set or clear the matching `{from,to}` link's `label`, `reason: 'missing'` when no such link exists — and carry the same unreachable-from-canvas caveat as city (tracked in [#495](https://github.com/acuhlmann/mermaid-gen/issues/495)). The Link-off flat kinds (orrery/river/archipelago/bridge/cycle/subway/iceberg) keep `renameEdge` refusing with `reason: 'not-graph'` — they have no free edge to rename; do not "fix" those.
...
- **Metaphor3D composite delegates per layer, links at the top.** `metaphorCompositeEdit.js` finds the layer owning an item id, runs that layer's `as` mutator on a mini `{ metaphor, items, links: [] }`, and merges items back. Add re-ids when a layer mutator mints an id already used in another layer. Delete purges composite-level `links[]` for every removed id (tree descendants included). Link appends to composite `links[]`, not a layer mini-doc — the fused planner reads only the top-level array. `renameCompositeEdge` reuses the shared `renameLinkedEdge`/`findLinkedEdge` helpers from `metaphorFlatItemsCore.js` (the same ones city and the flat kinds call) on that top-level array rather than delegating to a layer mutator — a composite link is never layer-scoped, so there is nothing to delegate to. Post-Add selection must use `result.metaphorKind` (the layer's `as`), not `JSON.parse(source).metaphor` (`"composite"`).
...
- **Mermaid timeline and pie identity is flat source order.** Both are ordered lists in the DSL, not edge graphs. Add inserts after the selected row and clones the source row's tail (` : 2004` on timeline, `: 386` on pie). Delete refuses the last row (`reason: 'last'`). Timeline hits use `g.timeline-node` (inner `path[id]`); pie hits use standard mermaid node ids when present, with index/`~label:` fallback through `graphEditIdFromDescriptor`. Link stays off.
- **Infographic compare uses a `compares` block, not `root`/`children`.** SWOT/quadrant/binary templates nest items under compare roots; Add appends under the selected root; Delete on a root refuses when it is the last top-level compare (`reason: 'last'`). Same AntV `data-indexes` scheme as hierarchy — do not route through `parseInfographicTree`.
