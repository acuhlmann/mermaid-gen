...
8. **Chart values** — shipped. Inline `spec.data.values` rows only (not `data.url`). Add inserts a sibling row after the selected datum index; Delete refuses the last row (`reason: 'last'`); Rename updates the primary nominal/string field for that row. Hit identity is Vega's datum index on `chart-mark` selections. `canLink: false`.
...
- **Flowchart parallel edges need disambiguation.** Two links between the same node pair are common (`A -->|first| B` and `A -->|second| B`). Delete/rename must pick **one** edge by its label or by Mermaid's `L_<from>_<to>_<n>` index (`parseFlowchartEdgeDataId` on the SVG path's `data-id`). When the canvas carries a label that no longer matches the source, the mutator returns `{ ok: false, reason: 'missing' }` — the same stale-label trap as sequence diagrams. Before the fix, delete/rename removed or relabeled **every** parallel edge between the pair.
