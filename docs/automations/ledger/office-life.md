...
| `ci-chromium-install-hangs`     | **filed** #625           | 2026-09-10. `.github/workflows/ci.yml`'s `test-server` step 4 ("Install Chromium (Anything runtime check)") stalled indefinitely on **two attempts and two runners** — ~30 min and ~19 min, steps 5–6 never started, no test body ran — while `main` had been green on the same step 90 min earlier. It has no `timeout-minutes`, so a transient apt/snapcraft stall becomes a six-hour hang rather than a two-minute failure, and `check` being `if: always()` means nothing goes red or green meanwhile. `routine:guard --reachable` answers **`NONE`** for that path, so it is filed against `improve` and deliberately not `ready-for-agent`. Proposed patch is in the issue; Chromium there is an optimization (the rung falls back to jsdom by design), so `timeout-minutes: 5` alone is the safe minimum.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
