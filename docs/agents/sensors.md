...
The smallest meaningful loop is `npm run check:affected` — it inspects your diff, runs **Prettier on changed files**, and runs only the other sensors that apply (including **`typecheck:strict` for `apps/server`** when server files change — loose `typecheck` alone misses strict-island regressions). Staged commits are auto-formatted by the Husky pre-commit hook (`lint-staged`).
