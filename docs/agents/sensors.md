...
| ESLint guidance drift            | `node scripts/verify-eslint-guidance-sync.mjs` (also via `test:scripts`)                                                                                             | `packages/eslint-config/formatter.cjs`'s inline `GUIDANCE` map drifting from `packages/eslint-config/guidance.js` — the formatter duplicates the map because ESLint loads formatters via `require()` and ESM↔CJS interop is uneven across versions                                                                                                                                                                                                                            | Prints missing/extra/mismatched rule ids; also enforced as a test via `test:scripts`, which is what selects it under `test:affected` when either file changes                                                                                                 |
| Mermaid version pins             | `npm run verify:mermaid-pins` (also via `test:scripts`)                                                                                                              | Mermaid's version disagreeing across its four declarations: the `mermaid` range in `package.json`, `apps/web/package.json` and `apps/server/package.json`, plus the `mermaid@<major>` CDN URL in `apps/server/src/mcp/apps/mcpAppDiagramPreview.js`. The CDN copy is invisible to npm, so nothing else notices a bump that missed it — and a lagging major leaves external MCP Apps agents rendering grammar the server gate rejects                                          | Names the lagging side and the value to set; also enforced as a test via `test:scripts`, selected under `test:affected` by any `package.json` or that App HTML file. Full coupling table in [`agent-blast-radius.md`](../agent-blast-radius.md)               |
...
| Routine budget                   | `npm run routine:guard -- --postflight <name>` (`-- --filings` for the queue)                                                                                        | A scheduled NFR routine exceeding its declared `maxFiles` or `maxIssues`, in debt on findings it filed itself (pay-before-file), writing outside `allowedPaths`, touching a don't-touch path, editing a playbook it does not own, deleting a test file outside prune's dead-pair allowance (the suite plus the unreferenced module it alone proves, in one commit — [`prune.md`](../routines/prune.md) § 2 gate 5), shrinking one, or declaring an unrecognised `mergePolicy` | Failure names the file and which budget rule it broke; file budgets live in the playbook front-matter, the filing-debt constants in the guard                                                                                                                 |
| Prune candidates (gates nothing) | `npm run prune:scan` (**not** in `check`; `-- --json`, `-- --list`)                                                                                                  | Tracked files nothing else in the repository refers to: a document unreachable from a root or loaded rule, a module no import/config/glob reaches, a root script nothing invokes                                                                                                                                                                                                                                                                                              | One line per hit with a copy-pasteable `proof:` command; it is the `prune` routine's queue, not a build failure — and dependency-cruiser's `no-orphans` warning is the second sensor it must agree with. See [`docs/routines/prune.md`](../routines/prune.md) |
