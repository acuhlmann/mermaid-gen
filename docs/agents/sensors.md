...
| Ratchet must not gate CI         | `node scripts/verify-ratchet-no-gate.mjs` (also via `test:scripts`)                                                                                                  | A `scripts/*.test.mjs` file asserting zero live `compareRatchet` violations after `measureAll(ROOT, …)` — which would fail every PR when a tracked metric regresses, contradicting "verify:ratchet gates no build"                                                                                                                                                                                                                                                            | Names the test file, line, and forbidden assertion; see `docs/routines/ledger/improve.md` (2026-08-21)                                                                                                                                                        |
