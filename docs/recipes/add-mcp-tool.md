...
5. **Document the tool** in `README.md` under the _MCP Apps_ table (if it has a UI surface) and the _Other MCP tools_ list (if not). Update [`docs/architecture-external-agents.md`](../architecture-external-agents.md) if the tool changes any of the named flows (join, handshake, proposal, insights).
...
- `README.md`, `docs/architecture-external-agents.md` — docs.
