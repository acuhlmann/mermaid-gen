...
- **Label declutter** — labels register with a screen-space pass that ranks by importance then nearness and fades the loser of an overlapping pair (`labelDeclutter.js`). **Unreadable beats contested** — a label the canvas clips or a panel covers yields instead of holding its box; `measureChromeRects` feeds the pass the panels' real rectangles (not the span-discounted safe area). Group names (district / bed / axle / cluster / line) and the accented item are pinned; pinning buys a laxer bar, not an exemption. On small screens the reading strip shows at most three legend axis chips plus a `+N` counter (full legend remains in fullscreen and the guided read).
