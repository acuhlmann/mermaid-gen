...
| `composite`   | ≤ 4 layers | Mounts 2–4 base metaphor scenes as `layers` (`as` + per-layer `items`) when the topic has complementary spatial stories (structure + flow, hub + domains, …). `layout: "adjacent"` (default) parks layers side-by-side; `"overlay"` stacks them. The agent may auto-pick this without an explicit “combine metaphors” ask; a single-shape topic still stays on one base kind. |
...
**In-fullscreen kind switching** — while the canvas is in native fullscreen, a kind-switcher pill in the overlay lets the viewer change the spatial metaphor (e.g. city → terrain) without leaving fullscreen. The transition re-maps item magnitudes and groupings to the new encoding axes via `switchMetaphorKind.js`. Picking **Composite** expands the current scene into a two-layer montage (primary kind + a complementary companion such as city + river). The metaphor agent may also emit multi-layer composites on ordinary prompts when the topic mixes complementary spatial grammars. The exit button (×) is also available in-fullscreen because the native browser fullscreen toggle disappears once the overlay surface takes over (`DiagramFullscreenOverlay.jsx`).
