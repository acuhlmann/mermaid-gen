...
2. User edits source or loads state; client syncs via `GET`/`POST /api/copilotkit/state` with `contentType`. Canvas **Add / Delete / Rename / Link** (Mermaid flowchart, mindmap, stateDiagram-v2, sequenceDiagram, classDiagram, erDiagram, timeline, and pie; Infographic tree, dagre, network, flat lists/sequences, chart-_, hierarchy-structure, and compare-_; Metaphor3D **tree**, **city**, **garden**, and the other flat base kinds — layercake, galaxy, machine, terrain, orrery, river, archipelago, bridge, cycle, subway, iceberg; Chart **inline data.values** rows) is a discrete `POST /api/copilotkit/user-edit` (`origin: user`) so it does not go through Monaco debounce or auto-fix. Family table: [`docs/canvas-graph-edit.md`](../canvas-graph-edit.md).
