...
| `FORMS_REPAIR_MAX_ATTEMPTS`                                                | Fast `2`, Quality `2`                             | Same full-agent fallback cap for Forms mode (model-authored A2UI). `FORMS_REPAIR_MAX_ATTEMPTS_FAST` / `FORMS_REPAIR_MAX_ATTEMPTS_QUALITY` can tune by profile. Forms has no single-shot fixer — repair is agent-only (attempt 2+ still climbs to Quality).                                                                                                                                                      |
