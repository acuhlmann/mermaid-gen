...
| `ANYTHING_RUNTIME_FALLBACK_TIMEOUT_MS`                                     | `max(budget, 6000)`                             | Budget (ms) for the jsdom fallback when the browser rung fails open. Its own clock on purpose: the fallback must cover a child-process spawn plus a jsdom import graph, which the browser's budget was never sized for.                                                                                                                                                                                         |
