...
**Runtime-safe JS craft** — the runtime check executes the page's scripts, so patterns that throw at load time burn a repair turn even when static lints pass. `anythingDesignGuide.js` teaches the recurring classes: guard `querySelector` / `getElementById` before property access (or run init after the queried nodes exist); call `getTotalLength()` only on **geometry elements** (`<path>`, `<rect>`, `<circle>`, `<ellipse>`, `<line>`, `<polygon>`, `<polyline>`) — what throws is calling it on a **non-geometry** node such as a `<g>` wrapper, the `<svg>` root, `<text>`, or a plain `<div>` (stroke-draw animations must target the shape being stroked, not its container); with `@lib:d3`, chain `.append` / `.attr` on selections — never reassign a selection to a DOM node and call `.append` on it; with `@lib:d3` force layouts, give every node a string `id` and use those same ids in link `source`/`target`, with `.force('link', d3.forceLink(links).id(d => d.id))` — mismatched names or numeric indices throw `"node not found"` at runtime; with `@lib:d3` drag, define every handler referenced in `.on('start'|'drag'|'end', fn)` **before** `.call(d3.drag()…)` — a `const`/`let` handler declared later throws a TDZ `ReferenceError`, and a missing binding throws `"… is not defined"`; declare `const`/`let` before first use anywhere in the script (reads above the declaration fail the same way); with `@lib:matter`, read **`body.isSleeping`** on each body — `Matter.Sleeping` exposes set/update helpers, not `isSleeping()`, so `Matter.Sleeping.isSleeping(…)` throws `"is not a function"`; KPI/dashboard **formatters belong as standalone functions** (`formatCurrency`, `fmtNum`) — call them directly on values, because data rows (`{ label, value }`) are plain objects and `k.fmt(k.value)` throws `"fmt is not a function"` unless you defined `fmt` on that object; inline `<script>` blocks (no `type="module"`) **cannot use top-level `await` or `import`** — wrap async setup in an `async function` and call it from `DOMContentLoaded`, or add `type="module"` (top-level `await` in a classic script is a `script_syntax` error). Corpus fixtures `runtime-svg-gettotallength`, `runtime-d3-force-link-mismatch`, `runtime-d3-drag-handler-missing`, and `runtime-fmt-on-data-row` pin the recurring runtime classes; add a corpus case when a new craft rule closes a generation-bench bucket. Both engines must agree on corpus expectations — a fixture that only rejects under jsdom (e.g. asserting `<rect>` lacks `getTotalLength` when real Chromium implements `SVGGeometryElement`) is a drift trap ([#393](https://github.com/acuhlmann/mermaid-gen/issues/393)).
