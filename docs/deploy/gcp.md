...
- **Memory: `--memory=1Gi`** (both `scripts/deploy-cloud-run.sh` and the deploy workflow set it; the script honours `MEMORY` to override). Cloud Run's default is 512 MiB, which was fine until the Anything runtime check started rendering pages in Chromium — peak RSS for a render is ~215 MB on top of Node's baseline. Note the browser **replaces** the jsdom child process rather than running beside it, so 1 GiB is the right size and 2 GiB would be paying for headroom nothing uses. Adding Chromium also grows the image by roughly 400 MB, which lengthens the cold-start window described above.
