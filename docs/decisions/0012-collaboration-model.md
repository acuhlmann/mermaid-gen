...
- **Pitch rendering is now a per-surface obligation.** Desk speech, Slop Chat, the inbox, walk-bys, huddle beats and — since the follow-up below — floor conversation all surface one. A surface that carries a remark and not its pitch is a bug, not a boundary.

  **Where on a surface it goes is decided by that surface's chrome, not by precedent.** The walk-by puts its Do-it in the speech bubble because a walk-by _has_ no card; `FloorTalkCard` puts it in the card because § 6 rule 12 says the replying half is chrome, and — the load-bearing reason — `FloorDeskSpeech` returns `null` outright under `hideBody`, which the bubble is rendered with as `!showSpokenText`. A Do-it on the balloon would appear and vanish with a captions preference that has nothing to do with whether somebody had an idea. Pinned by `officeFloorTalk.test.jsx`.
...
- **Floor pitch:** `components/officeFloor/FloorTalk.jsx` (`TalkPitch`), `lastInboundFrom` in `officeFloor/useFloorActivity.js` (one scan returns the line and its pitch together), threaded `OfficeFloor` → `FloorCardSlot` via the existing `conversation` bundle.
