...
- The floor ships **surface-by-surface** (substrate → walk-bys → arrival → set pieces → meetings → desk peeking → free roam → conversation → usable props) with desktop users losing nothing at any point — the phasing in `office-isometric-mode.md` §5 depends on rule 1.
...
All nine slices in `docs/office-isometric-mode.md` § 5 have shipped: the floor substrate, walk-bys embodied, the isometric arrival, set pieces at their locations, meetings in the glass room, desk peeking, free roam, conversation in the room, and props you can use.

- Renderer #2: `apps/web/src/components/OfficeFloor.jsx` + `components/officeFloor/` (`FloorStage`, `FloorRoom`, `FloorRoam`, `FloorProps`, `FloorSeat`, `FloorFigure`, `FloorWalker`, `FloorBubble`, `FloorDeskSpeech`, `FloorPlayer`, `FloorTopBar`, `FloorCardSlot`, `FloorScenes`, `FloorTalk`, `useFloorActivity`, `useFloorPresence`, `useFloorTalk`, `useFloorPropUse`, `useFloorKeyboard`, `useFloorAutoPan`, `useFloorWalker`, `useWalkAnimation`, `FloorPersonCard`, `FloorMeeting`, `FloorPeek`, `FloorArrival`, `isoArt`) + `components/OfficeFloor.css`; stage scaling in `hooks/useStageScale.js`
- **Only worked example of rule 3 — "diegesis duplicates, never replaces": usable props** (slice 9). The coffee machine on the floor pours a coffee break by calling the desk dock's own `getCoffee` verb (`utils/officeFloorProps.js` is the table saying so, and it has exactly one row with a verb in it). The labelled control in `DeskActionsDock` remains the primary path and is untouched, which is the whole of what rule 3 asks. The printer and whiteboard duplicate nothing and produce nothing — under the Sign-off rule that is the ceiling for a floor affordance, not a gap. Which props are usable is derived from the room (`propTileFor`), so the rule cannot rot into a hand-maintained list
...
- Tests: `apps/web/test/officeFloorPlan.test.js` (projection, its inverse, layout invariants, derived mark families, and a drift guard that every `CAST_TIERS` member has a seat), `apps/web/test/officeFloorMovement.test.js` (the snap ladder, the prop-mark roster and its nearest-prop tiebreak, and that no click anywhere on the plate puts you inside the leadership glass), `officeFloorRoam.test.jsx`, `officeFloorTalk.test.jsx` (the floor as a second renderer of the IM thread), `officeFloorProps.test.jsx` (that the machine fires the desk's own verb, exactly once), `useWalkAnimation.test.jsx` (walk interrupts, against a stubbed animation engine), `apps/web/test/officeFloor.test.jsx`, `officeFloorMeeting.test.jsx`, `officeFloorPeek.test.jsx`, `officeDeskWork.test.js`
