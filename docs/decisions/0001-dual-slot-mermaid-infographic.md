...
**Status:** Accepted (historical — session state has since grown to **five slots**: Mermaid, Infographic, Metaphor3D, Chart, and Anything; see [`docs/guide/content-types.md`](../guide/content-types.md) and [`CLAUDE.md`](../../CLAUDE.md). The dual-slot _pattern_ — independent `diagramSource` per `contentType` plus an `activeContentType` pointer — still applies.)
